...
def parse_args():
    p = argparse.ArgumentParser("Infer with D2Feat student")
    p.add_argument("--seq_a")
    p.add_argument("--seq_b")
    p.add_argument("--test_csv")
    p.add_argument("--out_csv", default="d2feat_submission.csv")
    p.add_argument("--checkpoint", required=True)
    p.add_argument("--device", default="auto")
    p.add_argument("--precision", choices=["auto", "fp32", "bf16"], default="auto")
    p.add_argument("--batch_size", type=int, default=8)
    p.add_argument("--max_len", type=int, default=1024)
    p.add_argument("--esm_dim", type=int, default=1280)
    return p.parse_args()
...
def main():
    ...
    model = DualBranchStudentPPI(vocab_size=26, embed_dim=64, hidden_dim=128, esm_dim=args.esm_dim).to(device)
    ...
    if args.seq_a and args.seq_b:
        ...
        rows = []
        bar = tqdm(dl, desc="d2feat infer", leave=False)
