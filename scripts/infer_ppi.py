...
@torch.no_grad()
def predict_pair(model: SECAIModel, seq_a: str, seq_b: str) -> float:
    logits = model([seq_a], [seq_b])
    return torch.sigmoid(logits).item()


@torch.no_grad()
def predict_csv(model: SECAIModel, test_csv: str, out_csv: str, batch_size: int, num_workers: int, binary: bool, threshold: float):
...
def main():
    ...
    if args.seq_a and args.seq_b:
        prob = predict_pair(model, args.seq_a, args.seq_b)
        pred = int(prob > args.threshold) if args.binary else prob
        print({"prob": prob, "pred": pred})
    elif args.test_csv:
        out_csv = args.out_csv
        predict_csv(model, args.test_csv, out_csv, args.batch_size, args.num_workers, args.binary, args.threshold)
